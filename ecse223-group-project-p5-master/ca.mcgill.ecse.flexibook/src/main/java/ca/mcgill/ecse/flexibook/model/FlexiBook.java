...
public class FlexiBook implements Serializable
{
  ...
  public int numberOfServices()
  {
	  int i;
	  int numOfServices=0;
	  for (i=0; i<bookableServices.size(); i++) {
		  if (bookableServices.get(i) instanceof Service) {
			  numOfServices++;
		  }
	  }
  ...
  public List<Service> getServices()
  {
	  ...
	  for(BookableService bService : bookableServices) {
		  if (bService instanceof Service) {
			  newServices.add((Service) bService);  
		  }
	  }
    return newServices;
  }
  ...
  {
	  int i=0;
	  List<ServiceCombo> newServiceCombos = new ArrayList<ServiceCombo>();
	  for(i=0; i<numberOfBookableServices(); i++) {
		  if (bookableServices.get(i) instanceof ServiceCombo) {
			    newServiceCombos.add((ServiceCombo) bookableServices.get(i));  
		  }
	  }
